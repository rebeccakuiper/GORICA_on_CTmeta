...
# or equivalently:
goricaResult <- goric(est, VCOV = VCOV, H1, type = "gorica", comparison = "complement")
summary(goricaResult)
#
## AIC
H0 <- "overallPhi12 == overallPhi21"
goricaResult_H0 <- goric(est, VCOV = VCOV, H0, type = "gorica", comparison = "complement")
...
for(g in 1:G){
  ...
  # AIC: H0 vs Hunc
  set.seed(123)
  results_g_H0 <- goric(est, VCOV = VCOV, H0, comparison = "complement", type = "gorica") 
...
for(g in 1:G){
  for(s in 1:S){
    ...
    # AIC: H0 vs Hunc
    set.seed(123)
    results_g_s_H0 <- goric(est, VCOV = VCOV, H0, comparison = "complement", type = "gorica") 
