...
Target_TI <- c(unique(TI)[order(unique(TI))], 5, 6, 7, 8, 9, 10)
G <- length(Target_TI)
        ...
        if(any(Re(eigen(Phi_VARest)$values) < 0)){
          ...
          if(any( eigen( CovMx[((s-1)*q^2+1):(s*q^2),((s-1)*q^2+1):(s*q^2)] )$values < 0 )){
            ...
            if(any( eigen( CovMx1[((s-1)*q^2+1):(s*q^2),((s-1)*q^2+1):(s*q^2)] )$values < 0 )){
              ...
              for(g in 1:G){
                Phi_VARest <- expm(-B_VARest*Target_TI[g])
...
for(teller_ES in 1:length(ES_Names)){
  for(teller_N in 1:length(N_Names)){ 
    for(teller_TI in 1:G){ 
      goricresult[nrow(goricresult) + 1,] = list("goric", HR[indexHtrue, teller_TI, teller_ES, teller_N], meanGORICAweights[indexHtrue, teller_TI, teller_ES, teller_N], q05GORICAweights[indexHtrue, teller_TI, teller_ES, teller_N], q95GORICAweights[indexHtrue, teller_TI, teller_ES, teller_N], indexHtrue, Target_TI[teller_TI], N_Names[teller_N], ES_Names[teller_ES]) 
      #goricresult[nrow(goricresult) + 1,] = list("goric", HR[indexHtrue, teller_TI, teller_ES, teller_N], meanGORICAweights[indexHtrue, teller_TI, teller_ES, teller_N], q05GORICAweights[indexHtrue, teller_TI, teller_ES, teller_N], q95GORICAweights[indexHtrue, teller_TI, teller_ES, teller_N], indexHtrue, Target_TI[teller_TI], N_Names[teller_N], (length(ES_Names)-teller_ES))   
    }
  }
}
...
#goricresult_all <- goricresult
#goricresult <- goricresult[which(goricresult$TI < 5), ]
